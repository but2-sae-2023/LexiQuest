            ...
            async for msg in ws:
                if msg.type == WSMsgType.TEXT:
                    ...
                    if decoded_msg is None:
                        ...
                        if msg_kind == 'join_waiting_room':
                            ...
                            if not waiting_room_name or waiting_room_name not in self._waiting_rooms:
                                ...
                                if not isinstance(client_identity, dict):
                                    ...
                                    """
                                    if waiting_room._game_state == 'en_cours':
                                        await client.waiting_room.cancel_client_id(client.id)
                                        await client.send_message('game_en_cours')
                                    """
                        ...
                        elif msg_kind == 'get_waiting_room_list':
                            await client.send_message('waiting_room_list', waiting_rooms = get_waiting_rooms_desc())
                            ...
                            if client.chat_session:
                                if client.chat_session.gameid is None:
                                    ...
                                    to_send = await self.hooks.on_chat_message(client.chat_session.id, client.id, gameid)
                                    for (addressee_id, body) in to_send.items():
                                        await client.chat_session.put_in_message_queue([addressee_id], 'new_game_start', sender=client.identity.get('name'), content=body)
                            else:
                                await client.send_message('error_new_game')
